 ...
 * Result is cached after the first definitive response (OK / FEATURE_NOT_SUPPORTED);
 * transient failures retry next launch. Returns null when unavailable (non-Play
 * install, OEM store, error).
 */
internal object InstallReferrerReader {
    ...
    fun fetch(context: Context, timeoutMs: Long = 3000): String? {
        // Blocks on a latch → must not run on the main thread (would deadlock with
        // the referrer callback if it's dispatched on the main looper).
                ...
                override fun onInstallReferrerSetupFinished(responseCode: Int) {
                    try {
                        if (responseCode == InstallReferrerClient.InstallReferrerResponse.OK) {
                            // OK = definitive answer received; mark before the getter
                            // so a transient binder throw doesn't force endless retries.
                            ...
                            // No Play referrer on this device — definitive (don't retry).
                            gotDefinitiveAnswer = true
                        }
        ...
        // Cache only on a definitive answer so a transient failure can retry later.
        if (gotDefinitiveAnswer) {
            // Only persist the referrer when it carries our token; no point storing
            // an organic-install referrer.
