...
- 사이드로드/Web 포함 **채널별 상세 표 + call-once(서버 atomic claim) 동작**은 [§3 채널별 통합](#3-채널별-통합) 서두 참조.
...
> 📦 npm: [`eodin-web`](https://www.npmjs.com/package/eodin-web) — 현재 **`1.0.0-beta.1`** (npm dist-tag `beta`).
...
> **이름 노트**: 4채널의 `@eodin/capacitor` 와 달리 web 채널은 unscoped `eodin-web` (npmjs.org 의 `@eodin` 조직 미생성 상태에서 ahn283 namespace 로 publish). 향후 `@eodin` 조직 생성 시 `@eodin/web` 으로 rename 가능 (deprecation alias 포함). `@eodin/capacitor` 는 npm 미발행 상태라 무관.

#### 3.5.1 의존성

```bash
# beta tag 사용 (권장 — 현재 stable 미릴리스)
npm install eodin-web@beta

# 또는 정확한 버전 pin (재현 가능 빌드)
npm install eodin-web@1.0.0-beta.1
```

`package.json` 에는 다음과 같이 들어간다:
...
`eodin-web` 는 client-only — `localStorage` / `navigator` / `document` / `history` 의존. SSR 환경에서 server-side 코드는 client component 안에서만 import / 호출:

```typescript
// components/EodinInit.tsx — Client Component
'use client';

import { useEffect } from 'react';
import { EodinAnalytics } from 'eodin-web';

export function EodinInit() {
  useEffect(() => {
...
import { EodinInit } from '@/components/EodinInit';
...
**환경변수 명명 규칙**:
- Next.js: `NEXT_PUBLIC_*` prefix (브라우저 노출 허용 표시)
- Vite: `VITE_*` prefix
- API key 가 클라이언트 노출되는 점은 본질적 한계 — backend 의 `apiKeyAuth` 가 origin allowlist 검증 강화 필요 (별도 ticket — `web-sdk/CHECKLIST.md` C3)
