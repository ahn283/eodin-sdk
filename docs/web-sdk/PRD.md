...
| 2026-05-03 (Phase 1.3 — P1 wire string anchor) | `EodinEvent.PageView = 'page_view'` 는 web 고유 추가이지만 향후 4채널이 동일 의미 이벤트 (앱 내 화면 진입) 추가 시 **반드시 동일 wire string `'page_view'` 사용**. `'screen_view'` 등 drift 금지. cross-channel funnel 분석 무결성 보장 |
