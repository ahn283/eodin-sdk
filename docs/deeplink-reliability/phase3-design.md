...
## 7.5 신뢰경계 (code-review 반영, M1/I1)

- **clickId 는 비밀이 아니다**: Play referrer 로 단말에 노출되어 같은 기기의 타 SDK 가 읽을 수 있다. 인증 없는 `getDeferredParams` 에서 `clickId` 직접 매칭이 가능하므로 **관측된 토큰으로 해당 deferred row 를 claim 할 수 있다.**
- 따라서 **deferred row 에는 PII·인증 컨텍스트를 절대 넣지 않는다** (현재: deeplinkPath/resourceId/utm — 저민감). row 에 민감 데이터를 추가하려는 변경은 이 경계를 재검토하는 트리거.
- 토큰 형식: `crypto.randomUUID()`(v4, 122bit) 채택 — 비밀이 아니므로 추측방어보다 충돌회피 목적. base64url ≥128bit 도 무방하나 표준 UUID 로 통일.
- claim/멱등: 같은 클릭의 재요청은 백엔드 `clickId @unique` + P2002 idempotent(Phase 3.1 H1) + 24h TTL 로 흡수.
