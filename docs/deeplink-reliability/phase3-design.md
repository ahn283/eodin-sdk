...
- **D1. clickId 생성 위치**: 서버(landing SSR 시 발급, 추측불가·중앙관리 — 권장) vs 클라.
- **D2. referrer 포맷**: 순수 토큰 `referrer=<clickId>` vs 구조화 `referrer=eodin_cid=<clickId>`(타 utm 공존). → 구조화 권장(확장성).
- **D3. Flutter Android 범위**: Flutter SDK 에도 Install Referrer 회수 추가(plori/tempy/arden/fridgify 가 Flutter라 **사실상 필수**) vs 네이티브 채널만. → **추가 권장**(아니면 주력 앱들이 결정론 혜택 못 받음).
- **D4. 토큰 만료/claim**: 기존 24h/claimed 유지 vs 조정.
- **D5. 비-Play 설치 fallback**: Phase 4 서버 확률 매칭으로 일원화 vs Android 도 잠정 fingerprint 유지.
